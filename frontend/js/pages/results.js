...
function renderResults() {
  ...
  <div class="page" style="max-width:740px;margin:0 auto;">
    ...
    <div class="card" style="margin-bottom:16px;padding:8px 0;">
      ...
      <!-- Attachments -->
      <div class="collapse-header" onclick="toggleCollapse('attach')">
        <div style="display:flex;align-items:center;gap:10px;">
          <span>📎</span>
          <span style="font-size:14px;font-weight:500;">Attachment Risks</span>
          <span class="badge ${r.attachmentFlags.length?'phishing':'safe'}">${r.attachmentFlags.length}</span>
        </div>
        <span style="color:var(--text3)">${state.collapseState.attach?'▲':'▼'}</span>
      </div>
      ${state.collapseState.attach?`
      <div class="collapse-body">
        ${r.attachmentFlags.length
          ? r.attachmentFlags.map(f=>`<span class="flag-pill attach">${f}</span>`).join('')
          : '<div class="empty-flag">No attachment risks detected.</div>'}
      </div>`:''}
    </div>
