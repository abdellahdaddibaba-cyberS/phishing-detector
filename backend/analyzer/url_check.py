...
def _check_virustotal(url: str) -> dict:
    ...
    if not api_key:
        return {"checked": False, "error": "No VIRUSTOTAL_API_KEY in .env"}
    ...
    try:
        ...
        if e.code != 404:
            print(f"[VT] GET error {e.code} for {url}")
    except Exception as e:
        print(f"[VT] Cache error: {e}")

    # Step 2: Submit for scan
    try:
        body = ("url=" + urllib.parse.quote(url, safe="")).encode()
        submit = _vt_post("/urls", body, api_key)
        analysis_id = submit.get("data", {}).get("id")
        if not analysis_id:
            return {"checked": False, "error": "No analysis ID"}
    except Exception as e:
        return {"checked": False, "error": f"Submit failed: {e}"}

    # Step 3: Better polling (longer + more attempts)
    print(f"[VT] Polling analysis {analysis_id} for {url}")
    for attempt in range(15):  # ~60-75 seconds max
        time.sleep(4 if attempt < 5 else 6)
        try:
            ...
            if status == "completed":
                ...
                positives = counts["malicious"] + counts["suspicious"]
                print(f"[VT] Completed! Positives: {positives}/{counts['total']}")
                return {
                    "checked": True,
                    "malicious": positives > 0,
                    "positives": positives,
    ...
    return {"checked": False, "error": "Analysis timeout - try again later"}
...
def analyze_urls(urls: list) -> dict:
    print(f"[URL Analyzer] Checking {len(urls)} URL(s): {urls}")
    if not urls:
