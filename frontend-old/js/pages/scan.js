// ============================================================
// PAGE: NEW SCAN
// Tabs: Full Email (.eml upload + paste) | Message Text | URL | Attachment
// ============================================================
function renderScan() {
  const tabs = [
  ...
  <div class="page" style="max-width:680px;margin:0 auto;">
    <div class="card">
      <div style="margin-bottom:20px;">
        ...
        <p style="font-size:13px;color:var(--text3);">Submit a full email or individual content to scan for phishing indicators across linguistic, URL, and attachment vectors.</p>
      </div>
      <div class="tabs">
      ...
      ${state.scanTab==='email'?`
        ...
        <div style="margin-bottom:18px;">
          ...
          <div style="background:rgba(0,212,170,0.06);border:1px solid rgba(0,212,170,0.15);border-radius:8px;padding:12px 14px;margin-top:10px;">
            ...
            <div style="font-size:12px;color:var(--text2);line-height:1.7;">✓ Full email body text<br>✓ All URLs<br>✓ Real attachment files (sent to VirusTotal)</div>
          </div>` : ''}
        </div>
        ...
        <div class="form-group">
          ...
          <textarea id="scan-email" rows="8" placeholder="Paste the full email content here (Ctrl+A in Gmail → Ctrl+C → paste here)&#10;&#10;Note: attachments won't be included this way — use .eml upload above for full analysis." style="resize:vertical;font-family:var(--mono);font-size:12px;">${state.scanEmail}</textarea>
      ...
      ${state.scanTab==='url'?`
        ...
        <div class="form-group">
          <label>Surrounding Context (optional)</label>
          <textarea rows="4" placeholder="Paste any text accompanying the URL..."></textarea>
        </div>
      `:''}
      ${state.scanTab==='attach'?`
        <div class="dropzone ${state.scanAttachName?'active':''}" id="dropzone" onclick="openFilePicker()">
          <div class="dropzone-icon">${state.scanAttachName?'📄':'📁'}</div>
          ${state.scanAttachName
            ? `<div style="font-size:14px;font-weight:500;color:var(--accent);">${state.scanAttachName}</div><div style="font-size:12px;color:var(--text3);margin-top:4px;">Click to change file</div>`
            : `<div style="font-size:14px;font-weight:500;">Drop file here or click to browse</div><div style="font-size:12px;color:var(--text3);margin-top:4px;">or drag and drop</div>`}
        </div>
        <p class="hint-text">Accepted: .exe · .js · .docm · .pdf · .zip · .bat · .vbs</p>
      `:''}
      ...
      <button class="btn btn-primary btn-lg" id="analyze-btn" onclick="startScan()" ${state.scanning?'disabled':''}>
        ${state.scanning ? `<span class="spinner"></span> Analyzing...` : '🔍 Analyze'}
      </button>
    </div>
    ...
    <div class="card" style="margin-top:16px;background:var(--surface2);">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;font-size:12px;font-family:var(--mono);">
        ...
        <div style="padding:10px;background:var(--surface3);border-radius:8px;">
          <div style="color:var(--text3);margin-bottom:4px;">Attachment</div>
          <div style="color:var(--danger);font-weight:700;">+${state.settings.attachWeight} pts</div>
        </div>
      </div>
